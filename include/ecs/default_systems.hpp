...
inline void RenderSystem(ComponentListSparse<Position>& positions,
                         ComponentListSparse<Scale>& scale,
                         ComponentListSparse<Rotation>& rotation,
                         ComponentListSparse<Render>& render) {
  ComponentIterator it(positions, scale, rotation, render);
  while (it.next()) {
    auto [pv, sv, rotv, rv] = it.get();

    float posToArr[3] = {pv.pos.x, pv.pos.y, pv.pos.z};
    rv.shaderProgram.setUniformValue(DataType::FLOAT_3, posToArr, "position");

    coma::Mat4 translate = coma::Mat4::GetTransform(
        pv.pos.x, pv.pos.y, pv.pos.z, sv.scale.x, sv.scale.y, sv.scale.y,
        rotv.rot.x, rotv.rot.y, rotv.rot.z);
    rv.shaderProgram.setUniformValue(DataType::MAT_4x4, &translate.m[0],
                                     "transform");
...

      {
        ...
        if (open) {
          ...
          if (ImGui::Checkbox("Mute", &muted)) {
            muted ? audio_comp.src.setGain(0.0f) : audio_comp.src.setGain(1.0f);
          }
