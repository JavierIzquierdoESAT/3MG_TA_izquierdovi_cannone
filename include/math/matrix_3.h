...
inline Mat3 Mat3::Multiply(const Mat3& other) const {
  ...
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 3; col++) {
      res.m[row * 3 + col] = m[row * 3 + 0] * other.m[0 * 3 + col] +
...
inline Mat3 Mat3::Adjoint() const {
  return Mat3(Vec3(m[4] * m[8] - m[5] * m[7], -(m[3] * m[8] - m[5] * m[6]),
                   m[3] * m[7] - m[4] * m[6]),
              Vec3(-(m[1] * m[8] - m[2] * m[7]), m[0] * m[8] - m[2] * m[6],
                   -(m[0] * m[7] - m[1] * m[6])),
              Vec3(m[1] * m[5] - m[2] * m[4], -(m[0] * m[5] - m[2] * m[3]),
                   m[0] * m[4] - m[1] * m[3]));
}

inline Mat3 Mat3::Transpose() const {
  return Mat3(Vec3(m[0], m[3], m[6]), Vec3(m[1], m[4], m[7]),
              Vec3(m[2], m[5], m[8]));
}
